...
public class ConfigController {
    ...
    public ResponseEntity<Object> getByName(@PathVariable("name") String name){
        if(!configService.existsByName(name))
            return new ResponseEntity<>(new Message("No existe una configuración para el nombre dado"), HttpStatus.NOT_FOUND);
        Config Config = configService.getOneByName(name).get(0);
        return new ResponseEntity<>(Config, HttpStatus.OK);
    }

    @PutMapping("/update/{id}")
    public ResponseEntity<?> update(@PathVariable("id") int id, @RequestBody ConfigDto configDto){
        if(!configService.existsById(id))
            return new ResponseEntity<>(new Message("No existe una configuración para id dado"), HttpStatus.NOT_FOUND);
        if(configService.existsByName(configDto.getName()) && configService.getOneByName(configDto.getName()).get(0).getId() != id)
        ...
        if(StringUtils.isBlank(configDto.getName()))
            return new ResponseEntity<>(new Message("La configuración debe de tener un nombre"), HttpStatus.BAD_REQUEST);
